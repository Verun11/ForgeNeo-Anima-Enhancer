...
def _parse_block_indices(block_indices_text: str, max_block_index: int) -> List[int]:
    ...
    for raw_part in text.split(","):
        ...
        if "-" in part:
            ...
            if len(pieces) != 2:
                raise RuntimeError(f"Invalid block range '{part}'. Use formats like '3-5' or '3,4,5,8'.")
            try:
                start = int(pieces[0])
                end = int(pieces[1])
            except ValueError:
                raise RuntimeError(f"Invalid block range '{part}'. Use integers like '3-5'.")
            ...
            try:
                ...
                raise RuntimeError(f"Invalid block index '{part}'. Use integers like '3' or ranges like '3-5'.")
    ...
    if not parsed:
        raise RuntimeError(
            f"No valid block indices found in '{block_indices_text}'. valid range: 0 to {max_block_index}"
        )
...
def apply_anima_patch(
    ...
    try:
        ...
        diffusion_model = getattr(patched_model.model, "diffusion_model", getattr(patched_model, "model", None))

    if not diffusion_model:
        return patched_model

    # Note: Forge handles checking diffusion model compatibility for Anima via _looks_anima_or_cosmos if needed
    if not _looks_anima_or_cosmos(diffusion_model):
        print("[AnimaReplay] Warning: Diffusion model does not look like Anima or Cosmos, patch might fail or do nothing if no matching blocks found.")
    ...
    try:
        ...
        print("[AnimaReplay] Could not find compatible block container, skipping patch.")
        return patched_model
        ...
        def replay_forward(*args, **kwargs):
            ...
            if not torch.is_tensor(out):
                if not warned_non_tensor["shown"]:
                    print(f"[AnimaReplay] Block {block_index} returned non-tensor output ({type(out)}). Falling back.")
                    warned_non_tensor["shown"] = True
                return out

            try:
                # ComfyUI forward uses self, so we might need to handle args correctly
                # We assume args is (hidden_states, ...) since orig_forward is bound to block
                return orig_forward(out, *args[1:], **kwargs)
            except Exception as e:
                if not warned_replay_error["shown"]:
                    print(f"[AnimaReplay] Replay failed on block {block_index} with {type(e).__name__}: {e}. Falling back.")
    ...
    def anima_unet_wrapper(model_function, kwargs):
        ...
        if do_actual:
            ...
            if spectrum_state["forecaster"] is None:
                spectrum_state["forecaster"] = FastChebyshevForecaster(m=spectrum_m, lam=spectrum_lam)
            ...
            if spectrum_state["forecaster"] is None or len(spectrum_state["forecaster"].H_buf) == 0:
                ...
                out = spectrum_state["forecaster"].predict(spectrum_state["cnt"], w=spectrum_w).to(kwargs["input"].dtype)
...
class ScriptAnimaEnhancer(scripts.Script):
    def title(self):
        return "Anima Detail Enhancer"

    def show(self, is_img2img):
        return scripts.AlwaysVisible

    def ui(self, is_img2img):
        with gr.Accordion("Anima Detail Enhancer", open=False):
            with gr.Row():
                enable_replay = gr.Checkbox(label="Enable Replay", value=True)
                enable_spectrum = gr.Checkbox(label="Enable Spectrum", value=False)
            
            with gr.Row():
                block_indices = gr.Textbox(label="Block Indices (comma separated or range)", value="3,4,5", placeholder="e.g. 3,4,5 or 3-5")
            
            with gr.Row():
                denoise_start_pct = gr.Slider(label="Denoise Start Pct", minimum=0.0, maximum=1.0, step=0.01, value=0.50)
                denoise_end_pct = gr.Slider(label="Denoise End Pct", minimum=0.0, maximum=1.0, step=0.01, value=1.00)
                
            gr.Markdown("**Spectrum Settings** (Only active if Enable Spectrum is checked)")
            with gr.Row():
                spectrum_w = gr.Slider(label="Spectrum weight", minimum=0.0, maximum=1.0, step=0.01, value=0.20)
                spectrum_m = gr.Slider(label="Spectrum M", minimum=1, maximum=32, step=1, value=16)
                spectrum_lam = gr.Slider(label="Spectrum lambda", minimum=0.0, maximum=100.0, step=0.01, value=0.50)
            with gr.Row():
                spectrum_warmup_steps = gr.Slider(label="Spectrum Warmup Steps", minimum=0, maximum=50, step=1, value=6)
                spectrum_window_size = gr.Slider(label="Spectrum Window Size", minimum=1, maximum=10, step=1, value=2)
                spectrum_flex_window = gr.Slider(label="Spectrum Flex Window", minimum=0.0, maximum=1.0, step=0.01, value=0.0)

        return [
            enable_replay, enable_spectrum, block_indices, 
            denoise_start_pct, denoise_end_pct, spectrum_w, 
    ...
    def process_before_every_sampling(self, p, *args, **kwargs):
        (enable_replay, enable_spectrum, block_indices, 
         denoise_start_pct, denoise_end_pct, spectrum_w, 
         spectrum_m, spectrum_lam, spectrum_warmup_steps, 
         spectrum_window_size, spectrum_flex_window) = args

        if not enable_replay and not enable_spectrum:
            return

        if not hasattr(p, "sd_model") or not hasattr(p.sd_model, "forge_objects"):
            print("[AnimaReplay] Cannot find forge_objects on p.sd_model. Not running in Forge? Skipping patch.")
            return

        unet_patcher = p.sd_model.forge_objects.unet.clone()
        
        apply_anima_patch(
            patched_model=unet_patcher,
